...
2. <<lite-pot>> /Lite-PoT: Practical Powers-of-Tau Setup Ceremony./
   *CCS 2025.*
...
3. <<SyncDKG>> /Synchronous Distributed Key Generation without Broadcasts./
...
4. <<SensorBFT>> /SensorBFT: Fault-Tolerant Target Localization using Voronoi Diagrams and Approximate Agreement./
   *ICDCS 2024*.
...
5. <<Delphi>> /Delphi: Efficient Asynchronous Approximate Agreement for Distributed Oracles./
   *DSN 2024*.
...
6. <<HashRand>> /HashRand: Efficient Asynchronous Random Beacon without Threshold Cryptographic Setup./
   *CCS 2024*.
...
7. <<Tor>> /Attacking and Improving the Tor Directory Protocol./
   *IEEE S&P 2024*.
...
8. <<E2C>> /EESMR - Energy Efficient State Machine Replication./
   *Middleware 2023*.
...
9. <<Apollo>><<Artemis>> /The unique chain rule and its applications./
...
10. <<OptRand>> /OptRand - Optimistically Responsive Reconfigurable Distributed Randomness./
    *NDSS 2023*.
...
11. <<OpenSquare>> /OpenSquare: Decentralized Repeated Modular Squaring Service./
    *CCS 2021*.
...
12. <<RandPiper>> /RandPiper – Reconfiguration Friendly Random Beacons with Quadratic Communication./
    *CCS 2021*.
    *Adithya Bhat*, Nibesh Shrestha, Aniket Kate, Kartik Nayak.
...
13. <<Reparo>> /Reparo - Publicly Verifiable Repair Layer for any Blockchain./
    *FC 2021*.
...
14. <<VTS>> /Verifiable Timed Signatures for Blockchains./
    *CCS 2020*.
